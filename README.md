...
- `builds/<buildId>.json` — the manifest for each deployed build of kovel.io, byte-identical to what `https://kovel.io/.well-known/kovel-build.json` served for that build. Each is also entered in the Sigstore Rekor log as a `hashedrekord` of the file's sha256.
- `builds/<buildId>.md` — the human-reviewed note: what changed and where.
- `latest.json` — the most recent build.
- `verify/` — the verifier: browser extension, CLI, and the shared comparison. Mirrored from the kovel.io source tree; the extension's `core.js` is byte-identical to `verify/core.mjs`.

Tags `deploy-<buildId>` pin the commit that added each manifest. History here is public; a rewrite would itself be evidence.
